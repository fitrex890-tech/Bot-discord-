...
def make_wheel_bar(segments: list, highlighted: int) -> str:
    """Rysuje pasek koła z podświetlonym segmentem — wskaźnik ▼ nad nim."""
    colors = [s["color"] for s in segments]
    # Linia wskaźnika
    pointer_line = "".join("▼" if i == highlighted else "　" for i in range(len(colors)))
    wheel_line = "".join(colors)
...
def slot_embed(a, b, c, title="🎰 Slot Machine", color=0xA855F7, result="", bet=0):
    ...
    if bet:
        e.set_footer(text=f"Stawka: {bet:,} 💎")
    return e
...
class BlackjackView(discord.ui.View):
    def __init__(self, user_id: int, bet: int, deck: list, player_hand: list, dealer_hand: list):
    ...
    async def hit(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = build_bj_embed(
            self.player_hand, self.dealer_hand,
            player_val, hand_value(self.dealer_hand), self.bet
        )
        await interaction.response.edit_message(embed=embed, view=self)
...
async def run_spin(interaction, user_id: int, bet: int, level: str, followup=False):
    ...
    def frame_embed(pointer: int, title="🎰 Koło Fortuny — kręci się..."):
        ...
        e.set_footer(text=f"Stawka: {bet:,} 💎 | Tryb: {level.upper()}")
        return e
    ...
    # Animacja — przyśpieszanie → zwalnianie → zatrzymanie na target
    total_frames = 20
    pointer = 0
    for frame_i in range(total_frames):
        t = frame_i / total_frames
        # speed: szybko na początku, wolno na końcu
        if t < 0.5:
            delay = 0.08
            step = 2
        elif t < 0.75:
        ...
        # Ostatnie 3 klatki — dociągaj do target
    ...
    # Upewnij się że wskaźnik stoi na target
    ...
    result_embed.set_footer(text=f"Stawka: {bet:,} 💎 | Tryb: {level.upper()}")
...
async def run_slot(interaction, user_id: int, bet: int, followup=False):
    ...
    # Bęben 1 kręci się
    for _ in range(7):
        s = random.choice(SLOT_SPIN_SYMS)
        await msg.edit(embed=slot_embed(s, "🎰", "🎰", bet=bet))
    ...
    for _ in range(7):
        s = random.choice(SLOT_SPIN_SYMS)
        await msg.edit(embed=slot_embed(r1["symbol"], s, "🎰", bet=bet))
        await asyncio.sleep(0.1)
    ...
    for _ in range(7):
        s = random.choice(SLOT_SPIN_SYMS)
        await msg.edit(embed=slot_embed(r1["symbol"], r2["symbol"], s, bet=bet))
        await asyncio.sleep(0.1)
    ...
    final = slot_embed(
        ...
        result=result_text, bet=bet
    )
    final.add_field(name="📊 Wypłaty (×3)", value=payout, inline=False)
    final.set_footer(text=f"Stawka: {bet:,} 💎 | ×2 takie same = ½ mnożnika")
...
class Games(commands.Cog):
    ...
    async def work(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title="💼 Praca wykonana!", description=f"{job_name}\n\n💰 **+{earn:,} 💎**", color=0x2ECC71)
    ...
    async def daily(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title="🎁 Daily odebrane!", description=f"💎 **+{rc}**\n🇵🇱 **+{rp} PLN**", color=0xF1C40F)
    ...
    async def spin(self, interaction: discord.Interaction, bet: int, level: app_commands.Choice[str]):
        ...
        if data["crypto"] < bet:
            return await interaction.response.send_message(f"❌ Za mało 💎. Masz: **{data['crypto']:,}**", ephemeral=True)
    ...
    async def blackjack(self, interaction: discord.Interaction, bet: int):
        ...
        if pv == 21 and dv == 21:
            ...
            return await interaction.response.send_message(embed=build_bj_embed(
                ...
                "🤝 Remis — oboje macie Blackjacka!", 0xF1C40F, True))
        if pv == 21:
            win = int(bet * 2.5)
            ...
            return await interaction.response.send_message(embed=build_bj_embed(
                player_hand, dealer_hand, pv, dv, bet, "done",
                f"🎉 BLACKJACK! **+{win:,} 💎**", 0x2ECC71, True))
        if dv == 21:
            return await interaction.response.send_message(embed=build_bj_embed(
                player_hand, dealer_hand, pv, dv, bet, "done",
                f"💀 Dealer ma Blackjacka! **-{bet:,} 💎**", 0xE74C3C, True))

        view = BlackjackView(interaction.user.id, bet, deck, player_hand, dealer_hand)
        await interaction.response.send_message(
            embed=build_bj_embed(player_hand, dealer_hand, pv, dv, bet),
            view=view
        )
    ...
    async def mines(self, interaction: discord.Interaction, bet: int, difficulty: app_commands.Choice[str]):
        ...
        if picked == "💎":
            ...
            embed = discord.Embed(title="💎 SAFE!", description=f"**+{win:,} 💎** (x{mult})", color=0x2ECC71)
        else:
        ...
        embed.set_footer(text=f"Stawka: {bet:,} 💎 | {mines_count}/{total} pól | x{mult}")
        await interaction.response.send_message(embed=embed)
    ...
    async def rob(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if remaining:
            return await interaction.response.send_message(embed=discord.Embed(
                title="⏳ Obserwowany!", description=f"Czekaj **{seconds_to_str(remaining)}**.", color=0xE74C3C
            ), ephemeral=True)
        victim = await db.get_profile(user.id)
        robber = await db.get_profile(interaction.user.id)
        if victim["crypto"] < 50:
            return await interaction.response.send_message(f"❌ {user.display_name} ma za mało 💎.", ephemeral=True)
        ...
        if random.random() < 0.45:
            ...
            embed = discord.Embed(title="🔫 Udany skok!", description=f"Okradłeś {user.mention}!\n**+{stolen:,} 💎**", color=0x2ECC71)
        else:
            ...
            if robber["crypto"] >= fine:
                ...
                fine = 0
            embed = discord.Embed(title="🚔 Złapany!", description=f"Nieudany napad!\nKara: **{fine:,} 💎**", color=0xE74C3C)
